...
function buttonStyle(value) {
	...
	var bg = util.unwrap_color(options.bgcolor || 'black');
	var fg = util.unwrap_color(options.fgcolor || 'white');
	var style = {
		'background': 'color(' + bg + ')',
		'color': 'color(' + fg + ')'
	};
	if (options.gradient) {
		style['background'] = 'linear-gradient(to bottom, color(' + bg + ' tint(10%)), color(' + bg + ' shade(15%)))';
		style['text-shadow'] = 'color(' + bg + ' shade(30%)) 0 1px'
	} else if (options['shade-gradient']) {
		style['background'] = 'linear-gradient(to bottom, transparent, rgba(0, 0, 0, 0.05) 40%, rgba(0, 0, 0, 0.1))';
	}
